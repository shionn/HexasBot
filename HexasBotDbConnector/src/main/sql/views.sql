...
SELECT t.id AS task, tp.date, t.type, t.url, p.marque, p.meta_model, p.name, tp.price, p.notify_channel
FROM task_price AS tp
...
SELECT p.id AS product_id, p.marque, p.meta_model, p.name, p.msrp,
  t.id AS task_id, t.url,
  tp.date AS last_price_date, tp.price AS last_price,
  DATE_FORMAT(`date`, '%Y-%m-%d %H') AS date_sort
FROM       product    AS p
